...
Module.register("AQI", {
	defaults: {
		...
		animationSpeed: config.animationSpeed, // One second
		debug: false,
		showLastUpdate: true,
	...
	getStyles() {
		return ["AQI.css", "weather-icons.css"];
	},
	...
	addIAQIRows(table) {
		...
		if (this.config.iaqi) {
			airQualityEntries.forEach(([key, data]) => {
				...
				nameCell.className = `iaqi-key small ${level.class}`;
				const mainName =
				...
				valueCell.className = "iaqi-value small";
		...
		if (this.config.weather) {
			...
			weatherEntries.forEach(([key, data]) => {
				...
				const nameCell = document.createElement("td");
				nameCell.className = `iaqi-key small weather-data`;
				nameCell.innerHTML =
					this.translate(`IAQI_${key.toUpperCase()}`) || key.toUpperCase();
				row.appendChild(nameCell);
				...
				const valueCell = document.createElement("td");
				valueCell.className = `iaqi-value small weather-data`;
