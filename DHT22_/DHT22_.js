Module.register('DHT22_', {
  ...
  socketNotificationReceived: function(notification, payload) {
    if (notification === 'DHT_DATA') {
      if (payload.humidity >= 0 && payload.humidity <= 100) {
        ...
        this.sendNotification("INDOOR_TEMPERATURE", payload.temperature);
        this.sendNotification("INDOOR_HUMIDITY", payload.humidity);
  ...
  getDom: function() {
    if (!this.config.showData) {
      document.querySelector(".DHT22_").style.display = "none";
    }
  ...
  //  wrapper.appendChild(header);
    ...
    if (this.config.layout === 'horizontal') {
      ...
      if (this.config.showTemperatureText) {
        temperatureDiv.appendChild(document.createTextNode(this.translate("Temperature") + " "));
      }
      if (this.config.showThermometerIcon) {
        const thermometerIcon = document.createElement('span');
        thermometerIcon.className = 'fa fa-thermometer ';
      ...
      if (this.config.showHumidityText) {
        humidityDiv.appendChild(document.createTextNode(this.translate("Humidity") + " "));
      }
